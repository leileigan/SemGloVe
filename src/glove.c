...
void *glove_thread(void *vid) {
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        if (feof(fin)) break;
        if (cr.word1 < 1 || cr.word2 < 1 || abs(cr.val) <= 1e-8) { continue; }
...
//        fprintf(stderr, "val: %f, log value: %.4f, diff: %.4f, fdiff: %.4f\n", cr.val, log(cr.val), diff, fdiff);
...
int save_params(int nb_iter) {
    ...
    if (use_binary > 0) { // Save parameters in binary file
        ...
        fprintf(stderr, "write word vectors to: %s", output_file);
...
int train_glove() {
    ...
    if (verbose > 0) fprintf(stderr, "learning rate: %lf\n", eta);
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        fprintf(stderr, "%s, iter: %03d, cost: %lf, num_lines: %lld, total_cost: %lf\n", time_buffer,  b+1, total_cost/num_lines, num_lines, total_cost);
